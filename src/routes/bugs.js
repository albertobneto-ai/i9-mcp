...
router.get('/stats', async (req, res) => {
  try {
    const [byStatus, bySection, byAutonomy, totals, done, blocked, inProg, hours] = await Promise.all([
      ...
      pool.query(`SELECT COUNT(*)::int as total, COALESCE(SUM(total_h),0)::numeric(7,2) as hours, COALESCE(SUM(dev_h),0)::numeric(7,2) as dev_h, COALESCE(SUM(margin_h),0)::numeric(7,2) as margin_h, COALESCE(SUM(unit_h),0)::numeric(7,2) as unit_h, COALESCE(SUM(func_h),0)::numeric(7,2) as func_h FROM bug_cards WHERE status != 'FORA_DO_ESCOPO'`),
      ...
      pool.query(`SELECT COUNT(*)::int as count FROM bug_cards WHERE blocked IS NOT NULL AND status != 'FORA_DO_ESCOPO'`),
      pool.query(`SELECT COUNT(*)::int as count FROM bug_cards WHERE status NOT IN ('DEPLOY_EXECUTADO','CORRECAO_APLICADA','EM_TESTES_UNITARIOS','EM_TESTES_FUNCIONAIS','CONCLUIDO','FORA_DO_ESCOPO')`),
