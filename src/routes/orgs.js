...
// POST /api/orgs/:id/metadata-retrieve — Retrieve a real zip from Salesforce for given types
// body: { types: [{ name: 'ExperienceBundle', members: ['AlgarPartner1'] }] }
router.post('/:id/metadata-retrieve', authMiddleware, async (req, res) => {
  try {
    const org = await getOrgById(req.params.id);
    if (!org) return res.status(404).json({ error: 'Org nao encontrada' });
